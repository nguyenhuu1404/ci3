...
class SitemapModel extends CI_Model {
	...
	public function getPages(){
		$data = $this->db->select('slug, created, category_type, modified')->from('categories')->where('is_menu', 1)->where('status', 1)->get()->result_array();
		return $data;
	}
	public function getPosts(){
		$data = $this->db->select('slug, created, modified')->from('news')->where_not_in('id', array(4, 6))->where('status', 1)->get()->result_array();
		return $data;
	}
	public function getCategories(){
		$data = $this->db->select('slug, created, category_type, modified')->from('categories')->where('is_menu =', 0)->where('status', 1)->get()->result_array();
		return $data;
	}
	public function getProducts(){
		$data = $this->db->select('slug, created, modified')->from('products')->where('status', 1)->get()->result_array();
		return $data;
	}
	public function getTags(){
		$data = $this->db->select('slug, created, type, modified')->from('tags')->where('status', 1)->get()->result_array();
		return $data;
	}
